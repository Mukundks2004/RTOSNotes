...
/*
   ----- 48450 -- lab handout -- advanced program for pork process  ------- 
...
Modified procsmess.c from Original source file available on http://mij.oltrelinux.com/devel/unixprg/
  ...
  --------------------------------------------------------------------------------------------
	Question 1 -- why the line 75, the printf (..) is shown early in the result screen?
	Question 2 -- why the line 40 is printed twice in the result screen?
...
int main() {
    ...
    if (childpid  == 0) {
        ...
        if (childpid == 0) {
            ...
            while ( waitpid(childpid, &status, WNOHANG) == 0 ) sleep(2);
...
This mac);
        ro queries the child termination status provided by the wait and waitpid functions. If the WIFEXITED macro indicates that the child process exited normally, the WEXITSTATUS macro returns the exit code specified by the child process.
